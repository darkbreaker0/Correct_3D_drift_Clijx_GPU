...
_fiji_dir = IJ.getDirectory("imagej")
_vkfft_default = os.path.join(_script_dir, "clijx", "src", "main", "native",
                              "vkfft_phase_correlation", "build", "clijx_vkfft.dll")
_vkfft_fiji = None
if _fiji_dir:
  _vkfft_fiji = os.path.join(_fiji_dir, "lib", "win64", "clijx_vkfft.dll")

if _vkfft_env is None or _vkfft_env == "":
  _candidate = None
  if _vkfft_fiji and os.path.exists(_vkfft_fiji):
    _candidate = _vkfft_fiji
  elif os.path.exists(_vkfft_default):
    _candidate = _vkfft_default

  if _candidate is not None:
    os.environ["CLIJX_VKFFT_PATH"] = _candidate
    JavaSystem.setProperty("CLIJX_VKFFT_PATH", _candidate)
    IJ.log("CLIJX_VKFFT_PATH set to " + _candidate)
  else:
    IJ.log("CLIJX_VKFFT_PATH not set and DLL not found in Fiji lib or default path")
else:
